...
nTrix<T> operator+(const nTrix<T>& lhs, const nTrix<T>& rhs)
{
	...
	{
		throw(std::invalid_argument(std::to_string(lhs.rows())));
	}
	if(lhs.cols() != rhs.cols())
	{
		throw(std::invalid_argument(std::to_string(lhs.cols())));
	}
...
nTrix<T> operator*(const nTrix<T>& lhs, const nTrix<T>& rhs)
{
	...
	nTrix<T> copy(lhs.rows(),rhs.cols());
	...
	for(int i = 0; i < copy.rows(); i++)
	{
		for(int j = 0; j < copy.cols(); j++)
		{
			...
			{
				sum += (lhs.m_matrix[left_row][k] * rhs.m_matrix[k][right_col]);
			}
			copy(i,j) = sum;
			sum = 0;
			right_col++;
